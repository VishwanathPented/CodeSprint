...
import {
  ...
  Trophy, RotateCcw, BookOpenText, Sparkles, Target
} from 'lucide-react';
...
export default function AptitudePractice() {
  ...
  useEffect(() => {
    const fetchSession = async () => {
      try {
        const url = `${API_URL}/aptitude/session?section=${section}&count=${count}` +
        ...
        if (res.ok && data.questions?.length) {
          ...
          setError(data.message || 'No questions available for this section.');
        }
      } catch {
  ...
  const handleSkip = () => {
    setAnswers((prev) => ({ ...prev, [current._id]: null }));
    handleNext();
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch(`${API_URL}/aptitude/grade`, {
        method: 'POST',
        headers: {
      ...
      if (res.ok) {
        setResult(data);
        if (user) updateProgress({ ...user, aptitudeProgress: data.aptitudeProgress });
      } else {
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center p-20 space-y-4">
        ...
        <p className="text-slate-500 font-medium">Loading your practice session...</p>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="max-w-2xl mx-auto p-8 text-center">
        ...
        <Link to="/aptitude" className="text-primary-600 hover:underline">← Back to Aptitude track</Link>
      </div>
    );
  }

  if (result) {
    return <ResultsScreen result={result} section={section} count={count} onRestart={() => navigate('/aptitude')} />;
  }
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
        <div className="p-5 border-b border-slate-100 dark:border-slate-800 flex items-center gap-2 flex-wrap">
          <span className={clsx(
            ...
            current.section === 'quantitative' && 'bg-blue-50 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300',
            current.section === 'logical' && 'bg-amber-50 text-amber-700 dark:bg-amber-900/30 dark:text-amber-300',
            current.section === 'verbal' && 'bg-emerald-50 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-300'
          )}>
            {current.section}
          </span>
        ...
        <div className="p-6 space-y-5">
          {current.passage && (
            <div className="p-4 bg-slate-50 dark:bg-slate-800/50 border-l-4 border-indigo-400 dark:border-indigo-600 rounded-r-md">
              <div className="flex items-center gap-2 mb-2">
                <BookOpenText className="text-indigo-500" size={14} />
                <span className="text-[10px] font-bold uppercase tracking-widest text-indigo-600 dark:text-indigo-400">Passage</span>
              </div>
              <p className="text-sm text-slate-600 dark:text-slate-300 leading-relaxed italic">{current.passage}</p>
            </div>
          )}
            ...
            {current.options.map((opt, i) => {
              ...
              return (
                ...
                >
                  ...
                  <span className={clsx(
                    'text-sm',
                    isSelected ? 'text-primary-900 dark:text-primary-200 font-semibold' : 'text-slate-700 dark:text-slate-300'
                  )}>
                    {opt}
                  </span>
                </button>
        ...
        <div className="bg-slate-50 dark:bg-slate-900 border-t border-slate-100 dark:border-slate-800 p-4 flex items-center justify-between gap-2">
          <button
            onClick={handlePrev}
            disabled={idx === 0}
            className="px-4 py-2 text-sm font-semibold text-slate-600 dark:text-slate-300 hover:bg-white dark:hover:bg-slate-800 rounded-md border border-slate-200 dark:border-slate-700 disabled:opacity-40 disabled:cursor-not-allowed flex items-center gap-1"
          >
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={handleSkip}
              className="px-4 py-2 text-sm font-semibold text-amber-600 dark:text-amber-400 hover:bg-amber-50 dark:hover:bg-amber-900/20 rounded-md border border-amber-200 dark:border-amber-800/50 flex items-center gap-1"
            >
              <Flag size={14} /> Skip
            </button>
            ...
            {idx < questions.length - 1 ? (
              <button
                ...
                className="px-5 py-2 text-sm font-bold text-white bg-primary-600 hover:bg-primary-500 rounded-md flex items-center gap-1 disabled:opacity-40 disabled:cursor-not-allowed"
              >
              ...
              <button
                ...
                disabled={submitting}
                className="px-5 py-2 text-sm font-bold text-white bg-emerald-600 hover:bg-emerald-500 rounded-md flex items-center gap-1 disabled:opacity-50"
              >
...
const QuestionTimer = ({ seconds, onExpire }) => {
  ...
  return (
    <div className={clsx(
      'flex items-center gap-2 px-3 py-1.5 rounded-md border font-mono text-xs font-bold',
      urgent
        ? 'bg-rose-50 border-rose-200 text-rose-600 dark:bg-rose-900/20 dark:border-rose-800 dark:text-rose-400 animate-pulse'
...
const ResultsScreen = ({ result, section, count, onRestart }) => {
    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className={clsx(
        ...
        <p className="text-sm opacity-80">
          {passed
            ? 'Above placement cutoff — keep this pace!'
            : 'Below typical cutoff (60%). Review the explanations and retry.'}
        </p>
      </div>
          ...
          <div
            ...
            className={clsx(
              ...
              r.isCorrect
                ...
                : 'border-rose-200 bg-rose-50/50 dark:bg-rose-900/10 dark:border-rose-800/50'
            )}
          >
      ...
      <div className="mt-8 flex flex-col sm:flex-row gap-3 justify-center">
        <Link
          to={`/aptitude/practice?section=${section}&count=${count}`}
          className="px-5 py-2.5 bg-slate-900 dark:bg-white dark:text-slate-900 text-white font-bold rounded-md flex items-center justify-center gap-2 hover:opacity-90"
          onClick={() => window.scrollTo(0, 0)}
        >
          <RotateCcw size={16} /> Retry Same Section
        </Link>
        <button
          onClick={onRestart}
          className="px-5 py-2.5 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 font-bold rounded-md flex items-center justify-center gap-2 hover:bg-slate-50 dark:hover:bg-slate-800"
        >
          Back to Track
        </button>
      </div>
